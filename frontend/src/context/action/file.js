...
export const uploadFile = (source=null, name, size=0, price=-1, link=null) => {
    //const body = JSON.stringify({...folder})
...
export const share_file = (id, file_id) => {
    ...
    return axios
        ...
        .catch(error => {
            console.log(error.response.data)
            let data = {message: {message: error.response.data.Error, code: "danger"}, status: 400}
            return data
        })
}
...
export const allFile = () => {
    return axios
        .get(`api/file/all-file/`, getHeader())
        .then(res => {
            let data = {status: 200, result: res.data}
            return data
        })
